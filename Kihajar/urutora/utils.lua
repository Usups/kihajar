...
function utils.drawWithShader(node, texture, x, y, data)
  ...
  if not node.enabled then love.graphics.setShader(utils.disabledImgShader) end
  if node.pointed then love.graphics.setShader(utils.pointedImgShader) end
  if node.pressed then love.graphics.setShader(utils.pressedImgShader) end

  utils.draw(texture, x, y, {centered = true, rotation = data.rotation, scale = data.scale})
